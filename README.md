...
- In Ciris `c1` succeds because `c` is chosen as an alternative to [missing] `b`; however `c2` fails because it considers the absence of `"b"` in the *composition* `(a product b)` to be a fatal error. A partially-specified configuration can reasonably be considered an error, so this is an entirely reasonable design choice.
...
- `Env` is
  - a `MonadError`, allowing for sequential composition with errors;
  - a `Parallel`, allowing for parallel composition (prefer this when you can!); and
  - a `Alternative`, allowing for alternative ("or else") composition. The empty case fails with `"Empty configuration."`.
